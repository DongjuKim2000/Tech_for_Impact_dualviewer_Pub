...
//        Log.d("get_bgurl", "${result}")
        ...
        val bg = try_catch_withLambdaFun({result:String -> JSONObject(result).getJSONObject("bgnow").getString("last")}, result)
        val time_LOCAL = try_catch_withLambdaFun({result:String -> convertUnixTimeToDateTime(JSONObject(result).getJSONObject("bgnow").getJSONArray("sgvs").getJSONObject(0).getLong("mills"))}, result)
        try_catch_withLambdaFun({result:String -> JSONObject(result).getJSONObject("bgnow").getJSONArray("sgvs").getJSONObject(0).getString("_id")}, result)
        val delta = try_catch_withLambdaFun({result:String -> JSONObject(result).getJSONObject("delta").getString("display")}, result)
        val arrow = try_catch_withLambdaFun({result:String -> JSONObject(result).getJSONObject("direction").getString("label")}, result)
        val iob = try_catch_withLambdaFun({result:String -> JSONObject(result).getJSONObject("iob").getString("display")}, result)
